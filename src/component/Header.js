import React, { useEffect, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { toggleMenu } from '../utils/appSlice';
import { YOUTUBE_SEARCH_API } from '../utils/constant';
import { addToCache } from '../utils/searchSlice';
import { Link, useNavigate } from 'react-router-dom';



const Header = () => {
    const SearchQ=useSelector(store=>store.search);
    const nevigate=useNavigate();
    const dispatch=useDispatch();
    const [searchResult,setSearchResult]=useState([]);
    const [showSearch,setShowResult]=useState(false);
    const [searchQuery,setSearchQuery]=useState("");
    const toggleMenuBtn=()=>{
       dispatch(toggleMenu());
    }
    const gotoSearchPage=(e)=>{
      if(e?.key==="Enter"){
        console.log(e);
        nevigate(`/search/${searchQuery}`);
      }
    }
    const searchResults=async ()=>{
      const data= await fetch(YOUTUBE_SEARCH_API+searchQuery);
      const json= await data.json();
      // console.log(json[1]);
      setSearchResult(json[1]);
      dispatch(addToCache({
           [searchQuery]:json[1]
      }
      ))
    }
    useEffect(()=>{
      let timer;
      if(SearchQ[searchQuery]){
        setSearchResult(SearchQ[searchQuery]);
      }
      else{
         timer=setTimeout(()=>searchResults(),200);
      }
      return ()=>{
        clearTimeout(timer)
      }
    },[searchQuery])

  return (
    <div className='bg-white fixed flex h-14 sm:h-20 gap-10 lg:gap-20 justify-between  p-5 shadow-lg w-screen z-20'>
    <div className='flex gap-4 items-center flex-shrink-0'>
        <img onClick={toggleMenuBtn} className='h-5 sm:h-8 cursor-pointer' src="data:image/png;base64," alt="hamburger menu" />
       <a href="/"> <img className=' ml-3 h-4 sm:h-6 ' src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRONuWvQHJt5ooi822FbtizsibO3m7cNHMHNg&usqp=CAU" alt="youtube" /> </a> 
    </div>
    <div className=' max-w[600px]  gap-4 flex-grow justify-center '>
    <form className='flex flex-grow max-w-[600px]'>
        <input 
         onKeyUp={gotoSearchPage()}
         value={searchQuery}
         onChange={(e)=>setSearchQuery(e.target.value)}
         onFocus={()=>setShowResult(true)}
         onBlur={()=>setShowResult(false)}
           className='w-full h-5 sm:h-8 border px-2 border-gray-300 rounded-l-full' type="text" />
       <Link to={"/search/"+searchQuery}><button  className='bg-gray-300 h-5 sm:h-8 rounded-r-full md:p-1'>
            <img className='h-5 sm:h-8 p-1 align-middle md:p-2' src="https://emojipedia.org/_next/static/media/search-dark.51fa5582.svg" alt="search" />
        </button>
        </Link>
        </form>
        { showSearch && <div className=' bg-white  px-2 max-w-[600px]'>
          <ul>
         { searchResult.map((name)=><li key={name}  className='hover:bg-gray-100 p-1'>{name}</li>
         )}
            
            
          </ul>
        </div>}
    </div>
    <div className='flex flex-shrink-0 md:gap-2'>
        <img className='h-5 sm:h-8' src="data:image/png;base64," alt="user" />
    </div>
    
    </div>
  )
}

export default Header